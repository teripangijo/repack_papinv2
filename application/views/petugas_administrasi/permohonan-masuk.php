<?php // application/views/petugas_administrasi/permohonan-masuk.php ?>
                                    ...
                                    <td class="text-center">
                                        <?php if(isset($p['id'])): ?>
                                            ...
                                            if (isset($p['status'])) {
                                                switch ($p['status']) {
                                                    ...
                                                    case '5': // Diproses Admin
                                                        // Tombol Proses
                                                        echo '<a href="' . site_url('petugas_administrasi/penunjukanPetugas/' . $p['id']) . '" class="btn btn-success btn-circle btn-sm my-1" title="Proses & Tunjuk Petugas Pemeriksa">
                                                                <i class="fas fa-user-plus"></i>
                                                              </a>';

                                                        // --- PERBAIKAN DITAMBAHKAN DI SINI ---
                                                        // Tombol Tolak Langsung
                                                        echo '<a href="' . site_url('petugas_administrasi/tolak_permohonan_awal/' . $p['id']) . '" class="btn btn-warning btn-circle btn-sm my-1" title="Tolak Langsung Permohonan">
                                                                <i class="fas fa-ban"></i>
                                                              </a>';
                                                        // --- AKHIR PERBAIKAN ---
                                                        break;
                                                    case '1':
                                                        echo '<a href="' . site_url('petugas_administrasi/penunjukanPetugas/' . $p['id']) . '" class="btn btn-warning btn-circle btn-sm my-1" title="Lihat/Edit Penunjukan Petugas">
                                                                <i class="fas fa-user-edit"></i>
                                                              </a>';
                                                        break;
                                                    case '2':
                                                        echo '<a href="' . site_url('petugas_administrasi/prosesSurat/' . $p['id']) . '" class="btn btn-primary btn-circle btn-sm my-1" title="Proses Penyelesaian Akhir Permohonan">
                                            ...
                                            if (isset($p['status']) && in_array($p['status'], ['0', '5', '1'])) {
                                                echo '<a href="' . site_url('petugas_administrasi/edit_permohonan/' . $p['id']) . '" class="btn btn-secondary btn-circle btn-sm my-1" title="Edit Data Pengajuan Permohonan">
                                                        <i class="fas fa-pencil-alt"></i>
                                                      </a>';
                                            }

                                            if (isset($p['status']) && in_array($p['status'], ['0', '5', '6'])) { // Ditambah status 6 agar bisa dihapus jika salah tolak
                                                echo '<a href="' . site_url('petugas_administrasi/hapus_permohonan/' . $p['id']) . '" class="btn btn-danger btn-circle btn-sm my-1" title="Hapus Permohonan" onclick="return confirm(\'APAKAH ANDA YAKIN ingin menghapus permohonan dengan ID Aju: ' . htmlspecialchars($p['id']) . ' atas nama ' . htmlspecialchars($p['NamaPers'] ?? 'N/A') . '?\nTindakan ini tidak dapat diurungkan!\');">
