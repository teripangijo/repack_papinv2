<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><?= isset($subtitle) ? htmlspecialchars($subtitle) : 'Daftar Pengajuan Kuota (Petugas Administrasi)'; ?></h1>
    </div>
                        ...
                        <?php if (!empty($pengajuan_kuota) && is_array($pengajuan_kuota)) : ?>
                            ...
                            <?php foreach ($pengajuan_kuota as $pk) : ?>
                                <tr>
                                    ...
                                    <td class="text-right"><?= isset($pk['requested_quota']) ? number_format($pk['requested_quota'],0,',','.') : '-'; ?> <?= htmlspecialchars($pk['satuan_barang'] ?? 'Unit'); ?></td>
                                    ...
                                    <td class="text-right"><?= (isset($pk['status']) && strtolower($pk['status']) == 'approved' && isset($pk['approved_quota'])) ? number_format($pk['approved_quota'],0,',','.') . ' ' . htmlspecialchars($pk['satuan_barang'] ?? 'Unit') : '-'; ?></td>
...
$(document).ready(function() {
    if (typeof $ !== 'undefined' && typeof $.fn.DataTable !== 'undefined') {
        $('#dataTablePetugasAdminPengajuanKuota').DataTable({ // ID diubah agar unik jika ada view admin lain
            "order": [[ 8, "desc" ]], 
            "language": {
